...
class HrAttendanceController (http.Controller):
    @http.route('/portal_attendance/_get_gmap_api_key', type='json', auth='public', website=True)
    def _get_gmap_api_key(self):
        key = request.env['ir.config_parameter'].sudo().get_param('portal_attendance.gmap_api_key')
        return json.dumps({
            'gmap_api_key': key or ''
        })

    @http.route('/portal_attendance/search_read/get_employee_data', type='json', auth='user', website=True)
...
class HrAttendancePortal(CustomerPortal):
    ...
    def portal_my_hr_attendances(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in='all', **kw):
        ...
        return request.render("portal_attendance.portal_my_hr_attendances", values)
    ...
    def portal_my_pos_order(self, attendance_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        return request.render('portal_attendance.portal_my_hr_attendance', values)
    ...
    def create_new_attendance(self, access_token=None):
        ...
        return request.render("portal_attendance.my_attendance_create_new", values)
